		...
		t.Run(tt.filename, func(t *testing.T) {
			...
			if tt.err != nil {
				...
				if tt.filename == "environmentresourcedatamodel.json" {
					...
					require.Equal(t, "myEnvValue", versioned.Properties.RecipeConfig.Env.AdditionalProperties["myEnvVar"])
					require.Equal(t, 2, len(versioned.Properties.RecipeConfig.Env.Secrets))
				}
